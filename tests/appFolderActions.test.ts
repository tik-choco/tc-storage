...
import { createFolderActions } from '../src/app/appFolderActions.js'
...
test('enabling folder sharing also enables auto-connect for the owner', () => {
  ...
  const actions = createFolderActions({
    ...
    folders: [folder],
    // `requestRoom` is gone: the engine now joins every room in `roomIds` simultaneously (see
    // p2p.ts), and `broadcastShare`'s second argument names the target room directly.
    networkRef: { current: { state: { mode: 'idle', rooms: [], peers: [], stablePeers: [], peersByRoom: {}, stablePeersByRoom: {}, lastEvent: '', messagesSent: 0, messagesReceived: 0 }, connect: async () => {}, disconnect: () => {}, broadcastShare: () => {} } },
    scheduleFolderSync: () => {},
    setBusy: () => {},
    setCurrentFolderId: () => {},
    setDeleteRequest: () => {},
