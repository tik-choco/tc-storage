...
import { browserSortModeKey, browserViewModeKey, canPreloadPreviewContent, isMediaFile, largeDownloadConfirmThresholdBytes, loadBrowserSortMode, loadBrowserViewMode, previewPreloadMaxBytes, requiresLargeDownloadConfirmation, shouldPreloadVisibleThumbnail } from '../src/app/appUtils.js'
...
test('browser sort mode defaults to manual and keeps valid preferences', () => {
  assert.equal(loadBrowserSortMode(), 'manual')

  localStorage.setItem(browserSortModeKey, 'updated-desc')
  assert.equal(loadBrowserSortMode(), 'updated-desc')

  localStorage.setItem(browserSortModeKey, 'unsupported')
  assert.equal(loadBrowserSortMode(), 'manual')
})
