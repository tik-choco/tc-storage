// Consumes the shared `books-backup` topic (published by tc-books on the same
// origin) and mirrors it as a single auto-updating file
// `tc-books-backup.json` in a dedicated "TC Books" folder.
//
// tc-books auto-publishes its full data bundle (journal/accounts/settings)
...
//    tc-books's data actually changes again).
...
//
// Contract: topic `books-backup` (v1); item shape is published by tc-books.
...
const booksBackupTopic = 'books-backup'
const inboxFolderName = 'TC Books'
const importedStateKey = 'tc-storage-books-backup-imported-v1'
const maxImportedEntries = 1000

/** One item published on the books-backup topic. Mirrors tc-books's publisher. */
...
interface ImportedBooksBackupEntry {
  checksum: string
  fileId: string
}

interface BooksBackupInboxOptions {
  ...
  resolveItem?: (item: BooksBackupItem, nodeId: string) => Promise<ResolveBooksBackupResult>
}
...
export type ResolveBooksBackupResult =
  | { kind: 'resolved'; bytes: Uint8Array }
  | { kind: 'permanent' }
  | { kind: 'transient' }

/** Parses and validates `meta.item` from a books-backup record. Returns undefined on any malformed field; never throws. */
export function parseBooksBackupItem(meta: Record<string, unknown>): BooksBackupItem | undefined {
...
export function loadImportedBooksBackupState(): Map<string, ImportedBooksBackupEntry> {
  try {
    ...
    const map = new Map<string, ImportedBooksBackupEntry>()
...
export function saveImportedBooksBackupState(state: Map<string, ImportedBooksBackupEntry>): void {
  const capped = [...state.entries()].slice(-maxImportedEntries)
  const entries: Record<string, ImportedBooksBackupEntry> = {}
  for (const [id, entry] of capped) entries[id] = entry
  try {
    localStorage.setItem(importedStateKey, JSON.stringify({ v: 1, entries }))
  } catch (error) {
    debugWarn('books-backup-inbox', 'failed to persist imported state', { error: describeError(error, 'unknown error') })
  }
}
...
async function resolveBooksBackupItem(item: BooksBackupItem, nodeId: string): Promise<ResolveBooksBackupResult> {
  ...
  try {
    ...
    debugWarn('books-backup-inbox', 'transient failure resolving item; will retry later', { id: item.id, cid: item.cid, error: describeError(error, 'unknown error') })
  ...
  try {
    ...
    if (checksum !== item.checksum) {
      debugWarn('books-backup-inbox', 'checksum mismatch, marking permanent', { id: item.id, cid: item.cid })
      return { kind: 'permanent' }
    }
    return { kind: 'resolved', bytes: plainBytes }
  } catch (error) {
    debugWarn('books-backup-inbox', 'failed to decrypt item, marking permanent', { id: item.id, cid: item.cid, error: describeError(error, 'unknown error') })
    return { kind: 'permanent' }
  }
}

export function createBooksBackupInboxActions(options: BooksBackupInboxOptions) {
  const { snapshotRef, setSnapshot, settingsRef, folderKeysRef, setFolderKeys, setFileContentCache, resolveItem = resolveBooksBackupItem } = options
  ...
  async function storeFileInMistBackground(file: FileRecord, folderId: string): Promise<void> {
    ...
    try {
      ...
      debugWarn('books-backup-inbox', 'background mist storage failed for books backup file', { fileId: file.id, error: describeError(error, 'unknown error') })
    }
  }

  async function importBackup(item: BooksBackupItem, bytes: Uint8Array, folderId: string): Promise<string> {
    const settings = settingsRef.current
    const now = new Date().toISOString()
  ...
  async function replaceBackupContent(item: BooksBackupItem, bytes: Uint8Array, previousFile: FileRecord, folderId: string): Promise<string> {
    const now = new Date().toISOString()
    const dataUrl = `data:${item.mimeType};base64,${bytesToBase64(bytes)}`
  ...
  async function runImport(record: SharedRecord): Promise<void> {
    const item = parseBooksBackupItem(record.meta)
    if (!item) return
    const state = loadImportedBooksBackupState()
    const existing = state.get(item.id)
    // Same checksum as last time: nothing changed. This also means a file
    // the user deleted stays deleted until tc-books's data actually changes
    ...
    if (result.kind === 'permanent') {
      ...
      saveImportedBooksBackupState(state)
      return
    }
  ...
  /** Imports the books backup from a books-backup record if its checksum changed. Safe to call repeatedly. */
