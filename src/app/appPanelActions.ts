...
interface PanelOptions {
  ...
  setJoinedRooms: SetState<JoinedRoom[]>
...
export function createPanelActions(options: PanelOptions) {
  const {
    ...
    setJoinedRooms, setNotice, setPendingShares, setPopoverPositions, setProfileOpen, setSelectedFileId,
  ...
  function acceptLinkedShare({ share, key }: LinkedShare) {
    ...
    // (see appFolderActions.ts) and must stay stable across accepting shares from other people.
    // A share for a different room instead joins that room alongside the home room -- both stay
    // reachable afterwards since the app now joins every room in `roomIds` simultaneously (see
    // p2p.ts and Settings' "joined rooms" list).
    if (share.roomId !== settings.roomId) {
      // Adding the room to joinedRooms is what drives roomIds -> useMistShare, which now joins
      // every room in the set simultaneously (see p2p.ts) -- no separate "make this the active
      // room" nudge is needed.
      setJoinedRooms((current) => upsertJoinedRoom(current, {
        roomId: share.roomId,
        label: share.senderProfile?.name?.trim() || share.folderName || share.fileName || share.roomId,
  ...
  function leaveJoinedRoom(roomId: string) {
    setJoinedRooms((current) => removeJoinedRoom(current, roomId))
    setNotice({ tone: 'info', text: '共有ルームから離脱しました' })
  }
  ...
  return { acceptLinkedShare, copyText, handlePreviewKey, leaveJoinedRoom, movePopover, movePreview, openFile, openFolderPanel, openFolderSharePanel, openProfile, openSettings, saveProfileDraft, saveSettingsDraft, selectFolder, showFileDetails, showFolderDetails, showFolderShare }
}
