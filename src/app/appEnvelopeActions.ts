...
import { mergeSnapshots, stampFilePatch, stampFolderPatch } from '../storage/crdt.js'
import { addActivity, stripFileContent, type FileRecord, type StorageSnapshot } from '../storage/domain.js'
import { canAutoImportFolderState, sharedFolderSignature, shouldDeferRemoteFolderStateImport } from '../folder/folderSync.js'
...
export function createEnvelopeActions(options: EnvelopeOptions) {
  ...
  function receiveFolderChange(envelope: ShareEnvelope) {
    ...
    if (envelope.changeType === 'folder-upserted') {
      if (envelope.folder?.shareEnabled && envelope.folder.id !== envelope.folderId) {
        return syncLog('folder-change folder upsert skipped: shared folder cannot be nested in another shared folder', envelopeLogDetails(envelope))
      }
  ...
  function applyRemoteFolderUpsert(envelope: ShareEnvelope) {
    ...
    if (folder.shareEnabled && folder.id !== envelope.folderId) {
      return syncLog('folder-change folder upsert skipped: shared folder cannot be nested in another shared folder', envelopeLogDetails(envelope))
    }
