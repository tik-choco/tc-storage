...
// A peer that never received a deletion and syncs after its tombstone was pruned locally can
// resurrect the item (mergeSnapshots treats an unseen id as new). 90 days keeps that rare while
// bounding snapshot growth from tombstones that nothing else ever prunes.
export const tombstoneRetentionDays = 90

// Drops file/folder records whose deletedAt is older than the retention window. Records without
// deletedAt, and records with an unparseable deletedAt, are always kept (conservative by design).
// Never mutates the input snapshot.
export function compactSnapshotTombstones(
  snapshot: StorageSnapshot,
  now: number,
  retentionDays: number = tombstoneRetentionDays,
): StorageSnapshot {
  const cutoff = now - retentionDays * 24 * 60 * 60 * 1000
  return {
    ...snapshot,
    folders: snapshot.folders.filter((folder) => !isStaleTombstone(folder.deletedAt, cutoff)),
    files: snapshot.files.filter((file) => !isStaleTombstone(file.deletedAt, cutoff)),
  }
}

// Emergency, local-persistence-only measure: drops every soft-deleted record regardless of age.
// Intended for a serialized copy when even trimmed activity cannot fit in remaining quota; callers
// must keep using the untouched in-memory snapshot so sync/merge tombstones are not lost.
export function dropAllTombstones(snapshot: StorageSnapshot): StorageSnapshot {
  return {
    ...snapshot,
    folders: snapshot.folders.filter((folder) => !folder.deletedAt),
    files: snapshot.files.filter((file) => !file.deletedAt),
  }
}

function isStaleTombstone(deletedAt: string | undefined, cutoff: number): boolean {
