...
export function ExpandedPreviewShell(props: ExpandedPreviewShellProps) {
  ...
  return (
    <section class={`preview-modal ${props.canZoom ? 'zoomable' : ''} ${props.flowEnabled ? 'flow-mode' : ''} ${idle ? 'idle' : ''}`} role="dialog" aria-modal="true" aria-label={props.file.name} onClick={props.closeFromEmptySpace}>
      ...
      >
        {props.flowEnabled ? (
          ...
          <PreviewContent file={props.file} loadingProgress={props.loadingProgress} expanded imageStyle={props.imageStyle} zoomable={props.canZoom} onSaveText={props.onSaveText} onSendFileToApp={props.onSendFileToApp} />
        )}
      </div>
