...
import { canPreloadPreviewContent, isImageFile } from '../app/appUtils.js'
import { ExpandedPreviewShell } from './ExpandedPreviewShell.js'
import { clamp, clampZoom, isImageLetterboxClick, isInteractiveFlowTarget, isLeftSideTap, isMobilePreview, pinchMetrics, pointInElement } from './previewInteraction.js'
...
export function ExpandedPreview(props: ExpandedPreviewProps) {
  ...
  const closeFromEmptySpace = (event: MouseEvent) => {
    ...
    // The preview-frame fills the body; its leftover (letterboxed) area around
    // the content should close too. For non-zoomable images the letterbox is
    // part of the <img> itself (object-fit: contain), so allow closing there.
    if (target?.classList.contains('preview-frame')) {
      props.onClose()
      return
    }
    // Images fill the frame with object-fit: contain, so the letterbox is part
    // of the <img>. Only close when the click lands on that letterbox, not on
    // the visible image, and not while zoomed in (a click there likely ends a
    // pan gesture).
    if (target?.tagName === 'IMG' && zoomRef.current.scale <= 1.02 && isImageLetterboxClick(target as HTMLImageElement, event)) props.onClose()
  }
  ...
  return (
    <ExpandedPreviewShell
      {...{
        ...
        onPrevious: props.onPrevious, onSaveText: props.onSaveText, onSendFileToApp: props.onSendFileToApp, setMode, total: props.total,
      }}
    />
  )
}
