...
import { stripFileContent, type FileRecord, type FolderBundle, type FolderRecord, type StorageSnapshot } from './domain.js'
import { describeError } from './errors.js'
import { downloadFile } from './fileIO.js'
...
interface FileContentOptions {
  ...
  finishDownloadProgress: (file: FileRecord, requestId: number) => void
  ...
  startDownloadProgress: (file: FileRecord, cached: boolean) => number
  startFileLoadProgress: (file: FileRecord) => string
  updateDownloadProgress: (file: FileRecord, percent: number, requestId: number) => void
}
...
export function createFileContentActions(options: FileContentOptions): FileContentActions {
  ...
  return { canResolveFileContent, downloadStoredFile, ensureFileContent, ensureFolderFilesStored, materializeFolderBundleFiles, preloadFileContent }
}
